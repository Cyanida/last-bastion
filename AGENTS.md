...
- **The when is the milestone.** A milestone is a release, named `v<major>.<minor>.<patch> – <theme>`:
  - a feature release has one theme, like `v0.8.0 – Balance, relics & bosses`;
  - a patch holds fixes and balance, like `v0.7.3 – Fixes & class balance`;
  - everything planned is before 1.0, which is a polished release (ROADMAP.md). After 0.9 comes 0.10;
  - `Ideas – not planned yet` is the inbox for new ideas that Jesse hasn't given a release yet.
  ...
  - The main AI opens the next patch for bugs. New ideas wait in the Ideas inbox until Jesse plans them.
...
2. **Milestone**, read from ROADMAP.md and the milestone descriptions (`gh api repos/Cyanida/last-bastion/milestones`):
   ...
   - a feature or balance change that fits the theme and scope of a planned release (ROADMAP.md): that milestone.
   - any other feature or idea: `Ideas – not planned yet`.
...
- **The Ideas inbox** (`Ideas – not planned yet`): new ideas wait there. Everything planned is before 1.0 (ROADMAP.md), and Jesse
  decides when an idea gets a release. If one fits a planned release, suggest it with `question-for-jesse`; don't create releases yourself.
