...
- `npm run test:perf` (after `npm run build`): the automated test, see below. Since v0.7.1 the run music plays through it, and it first checks that the music plays in every arena, that two stingers sound in each (all five kinds across the four arenas), and that the music never has more voices sounding than its budget.
