...
export const RELICS = {
  ...
  fireArrows: relic({ name: 'Fire Arrows', rarity: 'rare', icon: '🏹', family: 'flame', classId: 'archer', n: { perFocus: 0.04 }, n2: { perFocus: 0.06 }, a: { radius: 110, life: 3, dps: 10 },
    awaken: ['Rain of Cinders', (a) => `The Volley's area keeps burning for ${a.life} s.`], desc: (n) => `Arrow Volley's arrows each add a burn stack; burn damage +${pct(n.perFocus)} per Focus.` }),
  ...
  rimebow: relic({ name: 'Rimebow', rarity: 'rare', icon: '🎯', family: 'frost', classId: 'archer', n: { chill: 2, perFocus: 0.03, time: 3 }, n2: { perFocus: 0.04 }, a: { freeze: 0.5 },
  ...
  // ---------------------------------------------------------------- ⚡ Storm
  stormPennant: relic({ name: 'Storm Pennant', rarity: 'common', icon: '⚡', family: 'storm', n: { chance: 0.3, mult: 0.8, range: 170 }, n2: { chance: 0.4, mult: 0.9 }, a: { jumps: 2 },
  ...
  quicksilverSpurs: relic({ name: 'Quicksilver Spurs', rarity: 'common', icon: '🥾', family: 'storm', n: { per: 0.05, max: 10, time: 4 }, n2: { per: 0.07 }, a: { faster: 0.5 },
    awaken: ['Blur', (a) => `At full stacks your utility cools down ${pct(a.faster)} faster.`], desc: (n) => `Every chain or crit gives +${pct(n.per)} attack and movement speed for ${n.time} s (up to ${n.max} stacks).` }),
  ...
  thunderDrum: relic({ name: 'Thunder Drum', rarity: 'rare', icon: '🥁', family: 'storm', n: { damage: 150, radius: 170, mult: 0.5, range: 170 }, n2: { damage: 220 }, a: { delay: 1 },
    awaken: ['Rolling Thunder', (a) => `The thunderclap sounds again ${a.delay} s later.`], desc: (n) => `Using your ability sounds a thunderclap: ${n.damage}+ damage around you (grows with level), chaining from every enemy hit.` }),
  stormcallersHorn: relic({ name: "Stormcaller's Horn", rarity: 'legendary', icon: '📯', family: 'storm', n: { every: 15, mult: 3, radius: 70, range: 400 }, n2: { every: 12 }, a: { mult: 0.5, jumps: 3, range: 200 },
    awaken: ['Skyfury', (a) => `The strike chains to ${a.jumps} more enemies.`], desc: (n) => `Every ${n.every} kills a lightning strike hits the toughest enemy near you (${n.mult}× your hit).` }),
  galeforceQuiver: relic({ name: 'Galeforce Quiver', rarity: 'rare', icon: '🪶', family: 'storm', classId: 'archer', n: { per: 4, mult: 0.7, range: 170 }, n2: { per: 3, mult: 0.85 }, a: { every: 10, jumps: 5 },
    awaken: ['Gale Shot', (a) => `Every ${a.every}th arrow is a lightning bolt that chains ${a.jumps} times.`], desc: (n) => `Arrows pierce one more enemy per ${n.per} Focus, and every third arrow hit chains to another enemy (${pct(n.mult)}).` }),
  stormbornPelt: relic({ name: 'Stormborn Pelt', rarity: 'rare', icon: '🌩️', family: 'storm', classId: 'viking', n: { every: 4, perRage: 0.01, range: 170 }, n2: { every: 3 }, a: { perRage: 0.03 },
  ...
  bloodPact: relic({ name: 'Blood Pact', rarity: 'legendary', icon: '🩸', family: 'blood', mods: { damage: 1.4 }, mods2: { damage: 1.55 }, n: { bonus: 0.4, hp: 0.75 }, n2: { bonus: 0.55, hp: 0.8 }, a: { below: 0.5, heal: 0.01 },
    awaken: ['Covenant', (a) => `Below half HP kills restore ${pct(a.heal)} of your max HP.`], desc: (n) => `+${pct(n.bonus)} damage, but your max HP is cut to ${pct(n.hp)}.` }),
  wolfskin: relic({ name: 'Wolfskin Cloak', rarity: 'rare', icon: '🐺', family: 'blood', classId: 'viking', n: { per: 15 }, n2: { per: 10 }, a: { per: 0.05, max: 5 },
    awaken: ['Blood Frenzy', (a) => `Bleeding enemies you kill during Rage give +${pct(a.per)} attack speed for the rest of it (up to ${pct(a.per * a.max)}).`], desc: (n) => `During Berserker Rage your hits add a bleed stack, +1 per ${n.per} Rage.` }),
  ...
  guardiansAegis: relic({ name: "Guardian's Aegis", rarity: 'rare', icon: '🔰', family: 'holy', n: { every: 15, ward: 0.05 }, n2: { every: 12 }, a: { damage: 0.15 },
    awaken: ['Faithful', (a) => `While warded, +${pct(a.damage)} damage.`], desc: (n) => `Every ${n.every} s gain ward equal to ${pct(n.ward)} of your max HP.` }),
  ...
  reliquary: relic({ name: 'Reliquary of Saints', rarity: 'rare', icon: '⚱️', family: 'holy', classId: 'paladin', n: { perFaith: 0.02 }, n2: { perFaith: 0.03 }, a: { perFaith: 0.01 },
    awaken: ["Martyr's Relic", (a) => `Divine Shield also grants ward equal to ${pct(a.perFaith)} of your max HP per Faith when it ends.`], desc: (n) => `Every hit you take shaves ${n.perFaith} s × Faith off Divine Shield's cooldown.` }),
  seraphHalo: relic({ name: "Seraph's Halo", rarity: 'rare', icon: '🪽', family: 'holy', classId: 'angel', n: { base: 4, perGrace: 0.8, mult: 1.5 }, n2: { base: 6, perGrace: 1 }, a: { heal: 0.01 },
    awaken: ['Choir of Light', (a) => `The bolts heal you for ${pct(a.heal)} of your max HP each when they hit.`], desc: (n) => `Heavenly Radiance also fires ${n.base} + Grace × ${n.perGrace} light bolts in all directions.` }),
  hallowedBones: relic({ name: 'Hallowed Bones', rarity: 'rare', icon: '🦴', family: 'holy', classId: 'necromancer', n: { ward: 0.2, heal: 0.01, perSoul: 0.001 }, n2: { ward: 0.3 }, a: { leech: 0.005 },
  ...
  // ---------------------------------------------------------------- 💀 Grave
  soulLantern: relic({ name: 'Soul Lantern', rarity: 'legendary', icon: '🔮', family: 'grave', n: { chance: 0.1, max: 4, life: 9, hp: 40, damage: 8 }, n2: { chance: 0.15, max: 6 }, a: { damage: 14 },
  ...
  hexDoll: relic({ name: 'Hex Doll', rarity: 'rare', icon: '🪆', family: 'grave', n: { stacks: 1 }, n2: { stacks: 2 }, a: { range: 220 },
  ...
  gravediggersSpade: relic({ name: "Gravedigger's Spade", rarity: 'common', icon: '⚰️', family: 'grave', n: { per: 0.05, max: 5, radius: 150 }, n2: { per: 0.07 }, a: { every: 20, hp: 50, damage: 10, life: 20 },
  ...
  unbreakable: relic({ name: 'Unbreakable', rarity: 'legendary', icon: '🗿', family: 'steel', n: { every: 30, over: 0.25 }, n2: { every: 20 }, a: { armor: 0.5, time: 4 },
    awaken: ['Adamant', (a) => `After it blocks, +${pct(a.armor)} armor for ${a.time} s.`], desc: (n) => `Once every ${n.every} s, a hit that would take more than ${pct(n.over)} of your HP is blocked.` }),
  aegisFaithful: relic({ name: 'Aegis of the Faithful', rarity: 'rare', icon: '⛨', family: 'steel', classId: 'paladin', n: { per: 3 }, n2: { per: 2 }, a: { bonus: 0.5 },
...
const duo = <N extends Record<string, number>>(name: string, icon: string, families: [FamilyId, FamilyId], from: [RelicId, RelicId], desc: string | ((n: N) => string), n = {} as N): DuoDef =>
  ({ name, icon, families, from, desc: typeof desc === 'string' ? desc : desc(n), n });
export const DUOS = {
  ...
  boilingBlood: duo('Boiling Blood', '🫕', ['flame', 'blood'], ['salamanderScale', 'serratedEdge'], (n) => `Enemies that burn and bleed take both ticks ${pct(n.faster)} faster.`, { faster: 0.5, every: 0.5 }),
