...
/** v0.7.3 (#53): what is left of the signature ability's minimum downtime (g.player.vars['ability.readyAt']): refunds cannot cut the cooldown below it. */
export const cooldownFloor = (g: { player: { vars: Record<string, number> }; time: number }): number => Math.max(0, (g.player.vars['ability.readyAt'] ?? 0) - g.time);
