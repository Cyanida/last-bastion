...
const HOOKS: { [K in AbilityId]: AbilityHook<K> } = {
    ...
    tick(g, _c, dt) {
      ...
      if (has(p, 'sanctuary')) {
        ...
        if (crowded && (p.vars.sanctuary ?? 0) < p.abilityDur * n.maxExtend) {
          ...
          p.vars.sanctuary = (p.vars.sanctuary ?? 0) + dt * n.slowdown;
        }
      }
    },
    expire(g, c) {
      ...
      p.vars['ability.readyAt'] = g.time + downtime;
      ...
      const dmg = (attackDamage(base, p.stats.str, p.mods.damage) + (has(p, 'martyr') ? p.absorbed * U.martyr.n.mult : 0)) * (p.vars['shield.burst'] ?? 1); // v0.7.4 (#63): weaker when detonated early
  ...
  berserkerRage: {
    ...
    tick(g, c) {
      ...
      p.buff = {
        ...neutralBuff(),
        damage: b.damage + (p.vars.frenzy ?? 0),
        atkSpd: b.atkSpd,
    ...
    on: {
      onKill(g) {
        ...
        const n = U.frenzy.n;
        p.vars.frenzy = Math.min(n.cap + p.stats.secondary * n.capPerRage, (p.vars.frenzy ?? 0) + n.perKill);
      },
    },
    describe(p, c) {
  ...
  heavenlyRadiance: {
    activate(g, c) {
      ...
      if (has(p, 'benediction')) p.vars.cdRefund = Math.min(U.benediction.n.cap, slain * U.benediction.n.refund);
