...
export const STEEL_RELICS: Partial<Record<RelicId, RelicHooks>> = {
  ...
  shockSigil: {
    onDamageTaken(g, _ev, p) {
      ...
      if (g.time < (p.vars.shockReady ?? 0)) return;
      p.vars.shockReady = g.time + n.cooldown;
  ...
  unbreakable: {
    onIncoming(g, ev, p) {
      ...
      if (ev.blocked || g.time < (p.vars['unbreakable.ready'] ?? 0) || ev.amount <= p.hp * n.over) return;
      ev.blocked = true;
      p.vars['unbreakable.ready'] = g.time + n.every;
      credit(g, p, 'unbreakable', 'prevented', ev.amount, true);
      if (awakened(p, 'unbreakable')) p.vars['adamant.until'] = g.time + 4; // Adamant
    },
    tick(g, _dt, p) {
      if (g.time < (p.vars['adamant.until'] ?? 0)) bonus(p, 'armor', (p.cls.armor + p.mods.armor) * 0.5);
    },
  },
  ...
  ironhide: {
    tick(g, dt, p) {
      ...
      if ((p.vars['ironhide.t'] = (p.vars['ironhide.t'] ?? 0) + dt) < nOf(p, 'ironhide').every) return;
      p.vars['ironhide.t'] = 0;
