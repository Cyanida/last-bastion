...
export function updateEnemies(g: Game, dt: number): void {
  ...
  for (const e of g.enemies) {
    ...
    e.speed = e.baseSpeed * moon * (g.vars.enemySpeed ?? 1) * speedFactor(e.statuses) * (enraged(e) ? AFFIXES.enraged.n.speed : 1) * (e.buffT > 0 ? e.buffSpd : 1) * (e.phase >= 2 ? (e.def.p2SpeedMult ?? 1) : 1);
