...
export function showTestMode(setup: TestSetup, on: { start: (s: TestSetup) => void; play: (m: Mood) => void; stop: () => void; sting: (k: Stinger) => void; back: () => void }): void {
  ...
  const el = show(`
    <div class="panel dialog wide testmode">
      ...
      <div class="tm-grid">
        ...
        <label>Players <select id="tm-players">${options([['1', '1'], ['2', '2 · split screen'], ['3', '3 · P3 without controls'], ['4', '4 · P3-4 without controls']], String(setup.players ?? 1))}</select></label>
      </div>
  ...
  click(el, '[data-start]', () => on.start({
    classId: field('tm-class').value as ClassId, arena: field('tm-arena').value as ArenaId, act: num('tm-act', 1, FINAL.act), wave: num('tm-wave', 1, ACTS.length), level: num('tm-level', 1, 60), players: num('tm-players', 1, 4),
