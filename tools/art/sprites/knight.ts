/**
 * #157: the Armored Knight: blackened plate under a blood-red surcoat, a closed great helm with no plume, a plain red and black
 * heater shield (no gold, no sun: he must never read as the Paladin) and an arming sword brought down overhead.
 */
...
const kit: Kit = {
  ...
  body(f, torso, p) {
    f.part(torso, [[-6.8, -15.5], [6.8, -15.5], [7.6, -9], [7, -3], [-6.6, -3], [-7.4, -9]], 'black', 3); // breastplate
    ...
    f.part(torso, [[-4, -18.5], [4.6, -18.5], [5, -15], [-4.4, -15]], 'black', 3.6); // gorget
    const shield = torso.child(-6.5, -7 + 0.5 * p.sway, -0.05);
    f.part(shield, [[-5.4, -7], [5.4, -7], [5.4, 0], [3, 5], [0, 7.4], [-3, 5], [-5.4, 0]], 'red', 4.4, { trim: ['black', 1] }); // plain heater shield, per pale
    f.part(shield, [[0, -6], [4.4, -6], [4.4, 0], [2.4, 4.4], [0, 6.2]], 'black', 4.5, { profile: 'flat' });
  },
  head(f, head, p) {
    // a flat-topped great helm, closed: a dark eye slit across the face and breaths below it, no plume
    const slit: [number, number, 'black' | 'red', number][] = [1, 2, 3, 4, 5].map((x) => [x + 0.5, -5.5, x > 2 ? 'red' : 'black', x > 2 ? 3 : 0]); // a glint of red eyes
    const breaths: [number, number, 'black' | 'red', number][] = [[3.5, -2.5, 'black', 0], [4.5, -1.5, 'black', 0]];
    f.part(head, [[-4.8, 2], [-5.2, -6], [-4.6, -10.6], [4.6, -10.8], [6, -9], [6.4, -2], [5.4, 1.6], [0, 2.6]], 'black', 5, { details: [...slit, ...breaths] });
    f.part(head, [[-4.8, -6.6], [6.2, -6.6], [6.2, -6], [-4.8, -6]], 'black', 5.1, { profile: 'flat' }); // brow band
    void p;
  },
  weapon(f, grip, p) {
    if (p.smear) {
