...
export const sprite = robedSprite({
  id: 'abbot', S, tall: 63,
  robe: 'moss', panel: 'leather', trim: 'leather', hand: 'leather', fx: 'poison',
  head(f, head) {
    f.part(head, sc([[-5.4, 1], [-6, -5], [-4.6, -10], [0, -11.6], [4.4, -10], [6, -6], [5.8, 1], [0, 2.4]]), 'moss', 4.9, { folds: [0.4, 2.4, 1] }); // cowl
    f.part(head, sc([[-1.2, -8.2], [3.6, -8.2], [4.6, -5], [4.2, -1], [0.4, 0.6], [-1.6, -2]]), 'white', 5, { details: dt([[2.4, -5.8, 'poison', 5], [2.4, -5, 'darksteel', 0]]) }); // mask, a glass eye
  ...
  top(f, st, p) {
    ...
    f.part(st, E(4.4, -18.4, 2.2, 2.2), 'darksteel', 6.86, { details: dt([[4, -18.8, 'poison', 5]]) }); // censer
    f.part(st, E(3.4 - p.magic * 0.3, -22.2 - p.magic, 1.4 + p.magic * 0.4, 1.4 + p.magic * 0.4), 'poison', 6.87, { profile: 'flat', outline: false }); // its green smoke
  },
});
