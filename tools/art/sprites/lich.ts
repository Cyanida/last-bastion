/**
 * #158: The Lich. A skeleton king in torn purple robes, 1.2× the Paladin (66 art px tall with the crown; bosses draw at 4/3, about
 * 1.6× on screen): a skull under a gold crown, bone hands, and a staff topped with a soul orb. He floats: the robe hides his feet
 * and sways. His special is the hex: he raises the staff over the telegraph while purple soulfire gathers in his far hand, then
 * casts it down (shadow: purple).
 */
...
interface Pose {
  ...
  magic: number; // soulfire in the far hand (0: none)
  burst: number; // the cast: a burst of soulfire at the staff's foot (0: none)
}
const pose = (kw: Partial<Pose> = {}): Pose => ({ hip: [0, 0], lean: 0, head: 0, robe: 0, hem: 0, fist: [5, 12], staff: 0.08, far: [-2, 13], magic: 0, burst: 0, ...kw });

function lich(p: Pose): Figure {
  ...
  f.part(upF, sc([[-3, -1], [3, -1], [3.6, 8.5], [-3.6, 8.5]]), 'purple', 0.5, { dim: 1, folds: [0.4, 2.4, 0] });
  f.part(foF, sc([[-3.2, -0.5], [3.2, -0.5], [4.6, 6], [-4.6, 6]]), 'purple', 0.6, { dim: 1, folds: [0.4, 2.4, 0] }); // wide sleeve
  f.part(foF, sc([[-1.2, 5.5], [1.2, 5.5], [1.6, 9], [0, 10.4], [-1.6, 9]]), 'white', 0.7, { dim: 1 }); // bone hand

  // the robe, hip to ground, swaying
  const skirt = hip.child(0, -2 * S, p.robe);
  f.part(skirt, sc([[-7.5, 0], [7.8, 0], [10, 12], [11.5 + p.hem, 22], [8, 25], [5, 23.5], [2, 26], [-1, 24], [-4, 26], [-7, 24], [-10.5 + p.hem, 25], [-10, 12]]), 'purple', 1, { folds: [0.9, 3, 0] });
  f.part(skirt, sc([[-1.8, 0], [2.4, 0], [3.6, 24.5], [2, 26], [-1, 24], [-2.8, 24.8]]), 'darksteel', 1.1, { profile: 'flat', folds: [0.3, 2, 0] }); // dark panel

  f.part(torso, sc([[-8, -19.5], [8, -19.5], [9, -9], [8, -1], [-7.8, -1], [-9, -9]]), 'purple', 3.1, { folds: [0.5, 3, 0] }); // robe body
  f.part(torso, sc([[-8.4, -4], [8.6, -4], [8.6, -1.4], [-8.4, -1.4]]), 'gold', 3.5, { details: dt([[1.5, -2.7, 'soul', 5]]) }); // girdle with a gem
  f.part(torso, sc([[-11, -21.5], [10.5, -21.5], [12, -16], [9, -13.5], [4, -15.5], [-2, -13.5], [-8, -15.5], [-12, -14]]), 'purple', 3.8, { trim: ['gold', 1] }); // high collar mantle

  const head = torso.child(0.8 * S, -21 * S, p.head);
  f.part(head, sc([[-4.6, -1], [-5, -6], [-3.6, -9.6], [0, -10.6], [3.8, -9.6], [5.4, -6], [5, -2], [3.6, 1.6], [0, 2.2], [-2.8, 1.2]]), 'white', 5, {
    details: dt([[1.5, -5, 'darksteel', 0], [2.5, -5, 'darksteel', 0], [1.5, -4, 'soul', 6], [3.9, -5, 'darksteel', 0], [4.4, -4, 'soul', 5], [3.8, -2, 'darksteel', 1], [0.5, 0.5, 'darksteel', 1], [1.5, 0.5, 'white', 5], [2.5, 0.5, 'darksteel', 1], [3.4, 0.5, 'white', 5]]),
  }); // skull
  ...
  f.part(up, sc([[-3.2, -1], [3.2, -1], [3.8, 8.5], [-3.8, 8.5]]), 'purple', 7, { folds: [0.4, 2.4, 0] });
  f.part(fo, sc([[-3.2, -0.5], [3.2, -0.5], [4.6, 6], [-4.6, 6]]), 'purple', 7.1, { folds: [0.4, 2.4, 0], trim: ['gold', 1] });
  f.part(st, E(0.2, 0.2, 2, 2.2), 'white', 7.4, { details: dt([[1, -0.6, 'white', 2], [1, 0.8, 'white', 2]]) }); // bony grip

  if (p.magic) {
    const m = p.magic;
    f.part(world, ell(farH[0], farH[1] + 1, 2.5 + m * 1.6, 2.5 + m * 1.6, 16), 'soul', 9, { profile: 'round', outline: false });
    for (let k = 0; k < 3; k++) {
      const a = (k * 2 * Math.PI) / 3 + m;
      const r = 4 + m * 2.5;
      f.part(world, ell(farH[0] + r * Math.cos(a), farH[1] + 1 + r * Math.sin(a), 1.2, 1.2, 8), 'soul', 9.1, { profile: 'flat', outline: false });
    }
  }
  if (p.burst) {
    const [bx] = st.at(0, 16 * S), b = p.burst;
    f.part(world, ell(bx, GROUND - 1, b, b * 0.3, 28), 'soul', -1, { profile: 'flat', outline: false, dim: 1 });
    for (const [dx, h] of [[-0.7, 10], [-0.25, 16], [0.3, 13], [0.75, 8]])
      f.part(world, [[bx + dx * b - 1.8, GROUND - 1], [bx + dx * b + 1.8, GROUND - 1], [bx + dx * b * 1.15, GROUND - 1 - h * S]], 'soul', 8, { profile: 'flat', outline: false }); // soulfire tongues
  }
  return f;
}
...
// the hex: he rises, staff aloft, soulfire gathering (held to the end of the telegraph) | casts it down, the ground flares
...
export const sprite: SpriteDef = {
  id: 'lich', w: W, h: H, anchor: [X0, GROUND], tall: 66,
  anims: {
    idle: IDLE.map((p) => [220, lich(p)]),
    walk: WALK.map((p) => [120, lich(p)]),
    attack: ATTACK.map(([ms, p]) => [ms, lich(p)]),
    hurt: HURT.map(([ms, p]) => [ms, lich(p)]),
    death: DEATH.map(([ms, p]) => [ms, lich(p)]),
    special: SPECIAL.map(([ms, p]) => [ms, lich(p)]),
  },
  impact: 4,
  specialImpact: 3,
};
