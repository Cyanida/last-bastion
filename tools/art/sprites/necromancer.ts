...
function necromancer(p: Pose): Figure {
  ...
  f.part(torso.child(-3, -17, p.cape - p.lean), [[-1, 0], [4, 0], [3, 12], [2, 24], [-2, 27.5], [-5, 25], [-8, 27], [-11, 24], [-10, 14], [-6, 4]], 'purple', 0, { dim: 1, folds: [0.8, 3, 0] }); // cloak behind

  legs(hip, p.feet, (_th, sh, ft, z, dim) => {
    f.part(sh, [[-2.6, 0], [2.6, 0], [2.3, 11], [-2.3, 11]], 'purple', z, { dim, profile: 'flat', folds: [0.4, 3, 0] });
    f.part(ft, [[-2.4, -1.5], [2.2, -1.5], [5, 1], [5.8, 3], [-2.6, 3]], 'darksteel', z + 0.15, { dim }); // pointed boot
  });

  const skirt = torso.child(0, -3, p.skirt);
  f.part(skirt, [[-6.8, 0], [7, 0], [8.5, 12], [9, 19.5], [6, 18], [3.5, 20.5], [0, 18.5], [-3, 20.5], [-6, 18.5], [-9, 20], [-8.4, 12]], 'purple', 3.0, { folds: [0.6, 3, 0] }); // torn robe
  f.part(torso, [[-7, -17], [6.6, -17], [7.4, -10], [7, -3], [-6.6, -3], [-7.6, -10]], 'purple', 3.1, { folds: [0.4, 2.6, 0] });
  f.part(torso, [[-0.6, -17], [2, -17], [2.4, -3], [-1, -3]], 'purple', 3.15, { dim: 1, profile: 'flat' }); // front seam
  ...
  f.part(head, [[-5.6, 2], [-6, -4], [-5, -9], [-3, -13], [-1, -17], [1.5, -21 - p.plume * 10], [2.5, -15], [5, -9], [6.2, -4], [6, 1.5], [0, 3]], 'purple', 5, { folds: [0.4, 2.4, 1] }); // tall hood
  f.part(head, [[-1, 0.5], [0, -6], [3, -7.5], [5.2, -5], [5.2, 0.5], [2, 2]], 'darksteel', 5.1, { profile: 'flat', details: [[2.6, -4.3, 'soul', 5], [4.4, -4.3, 'soul', 5]] }); // shadowed face, green eyes
  ...
  f.part(fup, [[-2.6, -1], [2.6, -1], [2.2, 8], [-2.2, 8]], 'purple', 2.5, { dim: 1 });
  f.part(ffo, [[-2.4, -0.5], [2.4, -0.5], [3, 5], [-3, 5]], 'purple', 2.6, { dim: 1 });
  f.part(ffo, ell(0, 6.4, 1.6, 1.6), 'bone', 2.65, { dim: 1 }); // bony hand
  const fl = 1.6 + 1.8 * p.fx;
  f.part(new Bone(off[0], off[1] - 3 - fl), [[0, -fl * 1.8], [fl, 0], [0, fl * 0.8], [-fl, 0]], 'soul', 5.5, { outline: false }); // soul flame
  ...
  const cr = 2 + 0.8 * p.fx;
  f.part(st, [[0, -27 - cr * 2.2], [cr, -27 - cr], [0, -27], [-cr, -27 - cr]], 'soul', p.za, { details: [[0, -27 - cr * 1.4, 'soul', 6]] }); // crystal
  f.part(up, [[-2.6, -1], [2.6, -1], [2.4, 8], [-2.4, 8]], 'purple', p.za + 0.1);
  f.part(fo, [[-2.4, -0.5], [2.4, -0.5], [3, 5], [-3, 5]], 'purple', p.za + 0.2, { trim: ['bone', 0] }); // wide sleeve
  f.part(st, ell(0.3, 0.5, 2, 2), 'bone', p.za + 0.3); // bony fist
  if (p.fx > 0.9) f.part(st, ell(0, -27 - cr, cr * 2.4, cr * 2.4, 16), 'soul', p.za - 0.2, { profile: 'flat', outline: false }); // the crystal flares
  return f;
}
